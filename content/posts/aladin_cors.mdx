...
SOP에 의한 제약을 해결하기 위해 등장한 것이 바로 **CORS**다.
...
응답의 형식(Content-Type)이 **HTML** 또는 **XML** 또는 **JSON**인 경우 해당 응답은 [CORB](https://www.chromium.org/Home/chromium-security/corb-for-developers)에 의해 빈 응답으로 대체된다.
...
하지만 [Next.js 문서](https://nextjs.org/docs/advanced-features/custom-server), [Next.js github 예시](https://github.com/vercel/next.js/blob/canary/examples/with-custom-reverse-proxy/server.js), [블로그](https://medium.com/bb-tutorials-and-thoughts/next-js-how-to-proxy-to-backend-server-987174737331) 글 등을 참고해보니 내가 원하는 동작을 구현하는데 적절한 방법이 아닌 것 같았다.
