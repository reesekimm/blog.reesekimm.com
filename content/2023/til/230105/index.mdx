...
렌더링 성능을 최적화 할때 무작정 `React.memo`나 `useCallback` 등을 사용하기보다 state colocation을 먼저 고려해보는게 좋다. [[Reference]](https://kentcdodds.com/blog/state-colocation-will-make-your-react-app-faster)

Component
Context API / Redux 에도 적용된다.

서버 컴포넌트

[[Reference]](https://overreacted.io/ko/before-you-memo/)
...
React에서는 메모이제이션을 위해 `React.memo`, `useCallback`, `useMemo` 세 개의 API를 제공한다.

가장 최근의 입력값과 반환값을 메모이제이션하고 **참조 동등성**을 사용하여 컴포넌트의 리렌더링 여부를 결정한다.

> **참조 동등성 (referential equality)**\
> 값이 저장된 메모리 주소가 같으면 동일한 값으로 간주하는 것을 말한다.\
> React는 내부적으로 [Object.is](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is)를 사용

### React.memo로 리렌더링 줄이기

컴포넌트는 아래 조건에서 리렌더링 된다.

1. prop이 변하는 경우
2. state가 변하는 경우
3. 참조하고있는 context 값이 변하는 경우
4. 부모 컴포넌트가 리렌더링되는 경우
...
React DevTools로 `<Countbutton>`을 클릭하는 과정을 프로파일링 해보면 부모 컴포넌트가 리렌더링되면서 `<NameInput>`도 리렌더링 된 것을 알 수 있다.

![Profiling result - before using memo](./before-memo.png)

<Caption value="부모 컴포넌트가 리렌더링되면 자식 컴포넌트도 리렌더링 된다." />

하지만 `<CountButton>`을 클릭하더라도 `<NameInput>`이 참조하고 있는 상태값들(`name`, `setName`)은 클릭전과 동일하기 때문에 `<NameInput>`에 해당하는 **실제 DOM이 변경되지는 않는다**. 다시말해 `<NameInput>` 컴포넌트의 render, reconciliation 단계까지 수행했을때 virtual DOM에 변경사항이 없기 때문에 실제 DOM을 업데이트하는 commit 단계는 수행하지 않는 것이다.

**하지만 render, reconciliation 단계를 수행하는데 많은 연산이 필요하다면 어떻게 될까?** 실제 DOM이 변경되지 않음에도 불구하고 **불필요한 컴퓨팅 파워를 낭비**하게 되는 셈이다.
...
위 예시에서 버튼을 클릭하면 `<Parent>` 컴포넌트가 리렌더링 되고 `<SelectCandy>`도 함께 리렌더링 된다.

React.memo도 사용했고 selectedCandy 값도 그대로인데 왜 리렌더링이 됐을까?

원인은 onChange 속성으로 전달하는 showSelectedCandy에 있다.

showSelectedCandy 함수는 `<Parent>`가 렌더링될 때마다 새롭게 생성되어 별도의 메모리 공간에 할당되고 새로운 함수로 인식되기 때문이다. (참조 동등성)
