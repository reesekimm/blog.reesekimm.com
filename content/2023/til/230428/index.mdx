...
- **호환성** ⎯ HTML, CSS, SVG, React, Vue, Angular, Canvas, Three.js, jQuery, 그리고 모든 브라우저(Firefox, Chrome, Opera, Safari, IE)와 호환
- **성능** ⎯ GPU 가속을 포함한 여러가지 성능 최적화를 해주기 때문에 CSS animations/transition보다 빠르며, 성능 이슈 없이 부드러운 애니메이션 구현 가능
- **유연함** ⎯ GSAP은 애니메이션 구현에 필요한 도구(API, 플러그인, 프로퍼티 등)들 한데 모은 도구 상자임. 특정한 사용 패턴이나 아키텍처를 강제하지 않기 때문에 필요한 도구를 가져다 쓰기만 하면 됨
...
이럴 땐 임시 화면(이하 Preloader)을 활용하는 것이 적절한 방법일 수 있다. Home 화면 진입 시 Preloader를 먼저 표시하다가 이미지 로드가 완료되면 Preloader를 감추고 Home 화면을 표시하는 것이다. Preloader를 감출 때 애니메이션을 활용해서 전환 효과를 주면 보다 매끄러운 사용자 경험을 제공할 수 있다.
...
Preloader는 이미지 로드가 완료되면 complete 커스텀 이벤트를 발생시켜서 로드가 완료되었음을 알린다.
